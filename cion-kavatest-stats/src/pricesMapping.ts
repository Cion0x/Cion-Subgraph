...
} from '../generated/ChainlinkAggregatorWBTC/ChainlinkAggregator'
...
export function handleAnswerUpdatedWBTC(event: AnswerUpdatedEvent): void {
...
export function handleAnswerUpdatedWKAVA(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(WKAVA, event.params.current, event.block.timestamp);
}
...
// export function handleUniswapCionEthSwap(event: UniswapSwap): void {
//   let ethPerCion = -(event.params.amount0 * BigInt.fromI32(10).pow(18) / event.params.amount1) * BigInt.fromI32(100) / BigInt.fromI32(99)
//   let cionPrice = getTokenAmountUsd(WETH, ethPerCion)

//   let totalId = CION
//   _storeUniswapPrice(totalId, CION, cionPrice, "last", event.block.timestamp)

//   let id = CION + ":" + event.block.timestamp.toString()
//   _storeUniswapPrice(id, CION, cionPrice, "any", event.block.timestamp)
// }
